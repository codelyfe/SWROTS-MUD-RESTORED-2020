...
void make_scraps( OBJ_DATA *obj )
{
  ...
  if ( obj->carried_by )
  {
    ...
    if ( obj == get_eq_char( obj->carried_by, WEAR_WIELD ) &&  (tmpobj = get_eq_char( obj->carried_by, WEAR_DUAL_WIELD)) != NULL ) tmpobj->wear_loc = WEAR_WIELD;
...
/*	Using corpse cost to cheat, since corpses not sellable */
	...
	obj_from_char( obj );
	if ( IS_OBJ_STAT( obj, ITEM_INVENTORY ) || IS_OBJ_STAT( obj, ITEM_DEATHROT )) extract_obj( obj );
	else
