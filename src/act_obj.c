...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    if ( ( !CAN_WEAR(obj, ITEM_TAKE) || CAN_WEAR(obj, ITEM_INSTALLED) )
...
void do_put( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( container->item_type == ITEM_CONTAINER && IS_SET(container->value[1], CONT_CLOSED) ) || ( container->item_type == ITEM_LOCKER && IS_SET(container->value[0], CONT_CLOSED) ) )
	{
	    act( AT_PLAIN, "The $d is closed.", ch, NULL, container->name, TO_CHAR );
	    return;
	}
	...
	if ( container->item_type == ITEM_CONTAINER && (get_obj_weight( obj ) / obj->count)
	   + (get_obj_weight( container ) / container->count)
	   >  container->value[0] )
	{
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
   ...
   { 
	...
	case ITEM_WEAR_FINGER:
	    ...
	    if ( !get_eq_char( ch, WEAR_FINGER_L ) )
	    {
		if ( !HAS_BODYPART(ch, PART_HAND_L1) && !HAS_BODYPART(ch, PART_HAND_L2) && !HAS_BODYPART(ch, PART_HAND_R1) && !HAS_BODYPART(ch, PART_HAND_R2) )
		{
		 send_to_char( "&wWithout hands, how do you purpose to wear that?\n\r", ch );
		 return;
	  	}
                if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                {
	...
	case ITEM_WEAR_BODY:
	    ...
	    if ( !IS_NPC(ch) && obj->value[2] == LIGHT_ARMOR && !HAS_FEAT2( ch, ARMOR_LIGHT ))
	    {
		send_to_char( "You can't wear light armor until you are proficient with it.\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[2] == MEDIUM_ARMOR && !HAS_FEAT2(ch, ARMOR_MEDIUM) )
	    {
		send_to_char( "You can't wear medium armor until you are proficient with it.\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[2] == HEAVY_ARMOR && !HAS_FEAT2( ch, ARMOR_HEAVY) )
	    {
		send_to_char( "You can't wear heavy armor until you are proficient with it.&w\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[2] == POWERED_ARMOR && !HAS_FEAT2( ch, ARMOR_POWERED) )
	    {
		send_to_char( "You can't wear powered armor until you are proficient with it.&w\n\r", ch );
		return;
	    }
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	...
	case ITEM_WEAR_HEAD:
	    if ( ch->race == RACE_VERPINE || ch->race == RACE_TWILEK )
	    {
...
*/
	...
	case ITEM_WIELD:
	    ...
	    if ( !IS_NPC(ch) && obj->value[3] == WEAPON_BLASTER && !HAS_FEAT2( ch, WEAP_PROF_PISTOLS ) )
	    {
		send_to_char( "You cannot wield blasters until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[3] == WEAPON_BLASTER_RIFLE && !HAS_FEAT2( ch, WEAP_PROF_RIFLES) )
	    {
		send_to_char( "You cannot wield blaster rifles until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[3] == WEAPON_HEAVY && !HAS_FEAT2( ch, WEAP_PROF_HEAVY ) )
	    {
		send_to_char( "You cannot wield heavy weapons until you are proficient with them.&w\n\r", ch );
		return;
	    }
            if ( !IS_NPC(ch) && obj->value[3] == WEAPON_BOWCASTER && !HAS_FEAT2( ch, WEAP_PROF_BOWCASTER ))
            {
                send_to_char( "You cannot wield bowcasters until you are proficient with them.\n\r", ch);
                return;
            }
	    if ( !IS_NPC(ch) && obj->value[3] == WEAPON_FORCE_PIKE && !HAS_FEAT2( ch, WEAP_PROF_FORCEPIKE ) )
	    {
		send_to_char( "You cannot wield force pikes until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if (!IS_NPC(ch) && obj->value[3] == WEAPON_SIMPLE && !HAS_FEAT2( ch, WEAP_PROF_SIMPLE ) )
	    {
		send_to_char( "You cannot wield simple weapons until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && obj->value[3] == WEAPON_PRIMITIVE && !HAS_FEAT2( ch, WEAP_PROF_PRIMITIVE ) )
	    {
		send_to_char( "You cannot wield primitive weapons until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if ( !IS_NPC(ch) && ( obj->value[3] == WEAPON_LIGHTSABER || obj->value[3] == WEAPON_DUAL_LIGHTSABER ) && !HAS_FEAT2( ch, WEAP_PROF_LIGHTSABER ) )
	    {
		send_to_char( "You cannot wield lightsabers until you are proficient with them.&w\n\r", ch );
		return;
	    }
	    if ( tmpobj )
	    {
	...
	ch_printf(ch, "&W++\n\r");
		...
		case ITEM_WEAPON:
		  ch_printf(ch, "&z|| &wIt is a &G%s&w.  Average Damage: &G%d&w\n\r",
							obj->value[3] == WEAPON_VIBRO ? "vibro blade" :
		 ...
		 if(obj->value[3] == WEAPON_BLASTER || obj->value[3] == WEAPON_VIBRO || obj->value[3] == WEAPON_LIGHTSABER || obj->value[3] == WEAPON_FORCE_PIKE)
...
*/		int dam = fall_count*obj->weight/2;
		...
		if ( obj->in_room->first_person && number_percent() > 15 )
		{
			...
			damage( vch, vch, dam*vch->top_level, TYPE_UNDEFINED, FALSE );
		}
    	/* Damage objects */
	    switch( obj->item_type )
     	    {
