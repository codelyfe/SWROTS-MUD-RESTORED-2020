...
void do_detrap( CHAR_DATA *ch, char *argument )
{
    ...
    percent  = number_percent( ) - ch->top_level;

    separate_obj(obj);
    if ( !IS_NPC(ch) || percent > 0 )
    {
       send_to_char( "Ooops!\n\r", ch );
       spring_trap( ch, trap );
       return;
...
void do_dig( CHAR_DATA *ch, char *argument )
{
    ...
    if( !shovel )
     {
	send_to_char( "Perhaps you should invest in a shovel.\n\r", ch );
	return;
     }
...
void do_stun( CHAR_DATA *ch, char *argument )
{
    ...
    WAIT_STATE( ch, PULSE_VIOLENCE );
    fail = FALSE;
    chance = ris_save( victim, 10, RIS_PARALYSIS );
    if ( chance == 1000 )
      fail = TRUE;
    else
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
    ...
    WAIT_STATE( ch, PULSE_VIOLENCE );
    percent = number_percent( ) + victim->top_level - ch->top_level 
...
void do_checkmonitor( CHAR_DATA *ch, char *argument )
{
  ...
  chance = ( number_range(1,20) + ch->skill_level[COMPUTER_SKILL] + 
stat_table[get_curr_int(ch)].mod );
  if( chance < 17 )
   {
     sprintf( buf, "%s beeps in error.", monitor->name );
     act( AT_GREEN, buf, ch, monitor, NULL, TO_ROOM );
