...
char *  const   weapon_table    [12] =
{
    "none",
    "vibro",  "lightsaber", "blaster pistol", "blaster rifle",
    "heavy",  "bowcaster", "primitive", "force pike", "simple",
    "dual lightsaber", "slugthrower"
};
...
"bashed", "bashproof", "nomob", "window", "can_look", "autoclose" };
...
"sex", "null", "treat injury", "age", "height", "weight", "force", "hit", "move",
...
"detect_hidden", "weaken", "sanctuary", "faerie_fire", "infrared", "sorcery",
...
"wimpy", "pet", "train", "pazaak", "immortal", "deadly", "polyself",
...
"hasslug", "pageron", "notitled", "room", "r6", "r7", "r8", "r9", "r10", "r11", "r12", "r13",
...
"trip", "r7", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15", "r16", "r17", 
 ...
 */
	...
	if ( value < 0 || value > 10 )
	 {
	    send_to_char( "Feat range is 0 to 10.\n\r", ch );
	    return;
	 }
	victim->unused_feats = value;
	send_to_char( "done.\n\r", ch );
	return;
    }

    if ( !str_cmp( arg2, "reputation" ) )
    {
	if ( !can_mmodify( ch, victim ) )
	  return;
	if( IS_NPC(victim) )
	...
	if( value < 0 || value > 15 )
	 {
	   send_to_char( "Reputation range is from 0 to 15.\n\r", ch );
	   return;
	 }
	victim->reputation = value;
	send_to_char( "Done.\n\r", ch );
	if( ch != victim )
	  send_to_char( "Your reputation has been changed.\n\r", victim );
	return;
    }
    ...
    if ( !str_cmp( arg2, "frc" ) )
   {
    ...
    {
	if ( IS_NPC(victim) || !can_mmodify( ch, victim ) )
	  return;
	victim->skillpoints = value;
	send_to_char( "done.\n\r", ch );
	return;
    }
/*    if ( !str_cmp( arg2, "homeworld" ) )
    {
	if ( IS_NPC(victim) || !can_mmodify( ch, victim ) )
	  return;
	for( planet = first_planet; planet; planet = planet->next )
	 {
	   if( !str_cmp( planet->name, arg3 ) )
	    {
		victim->pcdata->home = planet->name;
		send_to_char( "Done.\n\r", ch );
		break;
	    }
	 }
	if( !planet )
	 {
	   send_to_char( "That is not a valid planet.\n\r", ch );
	   return;
	 }
	return;
    } */
	...
	if ( IS_NPC(victim) && IS_SET( victim->act, ACT_PROTOTYPE ) )
	{
	  ...
	  victim->pIndexData->hitroll = value;
	}
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "deadly, mountable, guardian, nokill, scholar, noassist, droid, nocorpse, noflee\n\r", ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
    ...
    if ( !str_cmp( arg2, "speaks" ) )
    {
	    ...
	    if ( !IS_NPC( victim ) )
	    {
    		int valid_langs = LANG_ARKANIAN | LANG_BASIC | LANG_BINARY |
				LANG_CEREAN | LANG_CHAGRI | LANG_DASHADI |
				LANG_DEFEL | LANG_DOSH | LANG_DRALLISH |
				LANG_FALLEEN | LANG_HODIN | LANG_HUTTESE |
				LANG_IKTOTCHESE | LANG_JAWA | LANG_KELDOR |
				LANG_MIRALUKESE | LANG_MON_CALAMARIAN |
				LANG_MUUN | LANG_NOGHRI | LANG_RODESE |
				LANG_RYN | LANG_SKYTRIC | LANG_SULLUSTESE |
				LANG_TOGRUTI | LANG_TOGORIAN | LANG_LEKKU |
				LANG_UBESE | LANG_UMBARESE | LANG_VERPINE |
				LANG_SRILUURIAN | LANG_SHYRIIWOOK;
...
*/
	...
	if ( !arg2 || arg2[0] == '\0' || !argument || argument[0] == 0 )
	{
	   send_to_char( "Usage: oset <object> affect <field> <value>\n\r", ch );
           ...
           send_to_char( "sex        tread injury age         height        weight       force\n\r", ch );
...
void do_cleanroom( CHAR_DATA *ch, char *argument )
{
 ...
 location = ch->in_room;
	...
	location->sector_type		= 1;
	location->light		= 0;
        obj2 = get_obj_index( location->vnum );
	if (obj2)
	  delete_obj( obj2);
        mob = get_mob_index( location->vnum );
	if (mob)
	  delete_mob( mob);
...
void do_rcopy ( CHAR_DATA *ch, char *argument )
{
  ...
  if( argument[0] == '\0' || !is_number(argument) )
   {
	send_to_char( "&GSyntax: &w&WRcopy <room to copy>\n\r", ch );
	return;
   }
  value = atoi(argument);
  if( ( from_room = get_room_index( value ) ) == NULL )
   {
	send_to_char( "&RThat is not a valid room.&w\n\r", ch );
	return;
   }
  if( ( in_room = get_room_index( ch->in_room->vnum ) ) == NULL )
   {
	send_to_char( "&RYou are not in a valid room.&w\n\r", ch );
	return;
   }
  in_room->description = from_room->description;
  send_to_char( "Done.\n\r", ch );
  return;
}
