...
JEDI_DATA * first_council;
JEDI_DATA * last_council;
		      ...
		      OBJ_DATA *obj, void *vo, CHAR_DATA *rndm )
{
  ...
  *pchck = '\0';
  ...
  if ( chkchar )
  {
    ...
    if ( !str_cmp(chck, "isconvict" ) )
    {
      if( IS_SET(chkchar->act, PLR_CONVICT) )
	return TRUE;
      return FALSE;
    }
    ...
    if ( !str_cmp(chck, "senator") )
    {
      SENATE2_DATA *senate;
 
      for( senate = first_senate; senate; senate = senate->next )
       {
	if( !str_cmp( chkchar->name, senate->leader ) )
	 return mprog_seval(senate->name, opr, rval, mob);
 	if( !str_cmp( chkchar->name, senate->number1 ) )
	 return mprog_seval(senate->name, opr, rval, mob);
	if( !str_cmp( chkchar->name, senate->number2 ) )
	 return mprog_seval(senate->name, opr, rval, mob);
       }
      return FALSE;
    }
    if ( !str_cmp(chck, "quest" ) )
    {
      ...
      if( IS_NPC(chkchar) )
	return FALSE;
      for( quest = chkchar->first_quest; quest; quest = quest->next )
	if( !str_cmp( quest->name, rval ) )
          if( quest->value < 1 )
            return TRUE;
      return FALSE;
    }
    if ( !str_cmp(chck, "hasquest") )
    {
      QUEST_DATA * quest;
	...
	return TRUE;
      for ( council = first_council; council; council = council->next )
       {
	if ( !str_cmp( chkchar->name, council->leader ) )
	    return TRUE;
	if ( !str_cmp( chkchar->name, council->number1 ) )
	    return TRUE;
       }
      return FALSE;
    }
