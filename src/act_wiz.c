  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
	...
	ch_printf( ch, "You have authorized %s.\n\r", victim->name);
     ...
     /* Below sends a message to player when name is accepted - Brittany   */                           
        ...
        "You are now fully authorized to play SWRotS.\n\r",victim->name);                               /* B */
     return;
     }
...
//    ch_printf( ch, "&GCargo Flags: &W%s\n\r",
//	flag_string(location->cargo_flags, c_flags) );
    if( IS_SET(location->room_flags2, ROOM_CARGOSHOP ) )
     {
      ch_printf( ch, "&GArt: &w%d  &GBacta: &w%d &GSemiGems: &w%d\n\r", location->art, location->bacta, location->semi_gem );
      ch_printf( ch, "&GRareGems: &w%d  &GCommOre: &w%d  &GRareOre: &w%d\n\r", location->prec_gem, location->comm_ore, location->rare_ore );
      ch_printf( ch, "&GSpice: &w%d  &GSlaves: &w%d  &GWeapons: &w%d\n\r", location->spice, location->slaves, location->weapons );
     }
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
 ...
 if(!IS_NPC(victim) && victim->desc )
  ...
  ch_printf( ch, "&W&z|       &GBank&W: %-10d            &GAC&W: %-5d             &GMove&W: %5d/%-5d    &z|\n\r", (IS_NPC(victim) || !victim->pcdata->bank) ? 0 : victim->pcdata->bank, GET_AC(victim), victim->move, victim->max_move);
  ch_printf( ch, "&W&z|   &GTopLevel&W: %-2d            &GDarkpoints&W: %-2d         &GForcepoints&W: %-2d             &z|\n\r", victim->top_level, IS_NPC(victim) ? victim->alignment : victim->darkpoints, IS_NPC(victim) ? 0 : victim->forcepoints );
 if(!IS_NPC(victim))
 ...
 if(IS_NPC(victim))
  ...
  ch_printf( ch, "&GSpeaking&W: " );
  for( lang = 0; lang < 30; lang++ )
   {
    if( victim->speaking == 0 || IS_SET(victim->speaking, lang_array[lang]) )
      ch_printf( ch, "%s ", lang_names[lang] );
   }
  send_to_char( "\n\r", ch );
  ch_printf( ch, "&GLanguages&W: " );
  for( lang = 0; lang < 30; lang++ )
   {
    if( victim->speaks == 0 || IS_SET(victim->speaks, lang_array[lang]) )
      ch_printf( ch, "%s ", lang_names[lang] );
   }
  send_to_char( "\n\r", ch );
 if(IS_NPC(victim))
 ...
 if(!IS_NPC(victim))
  {
   if(victim->pcdata->trials == HAS_PASSED)
    ch_printf( ch, "&GTrials&W: Has passed.\n\r" );
   else
    ch_printf( ch, "&GTrials&W: Not passed.\n\r" );
   if(victim->pcdata->has_trained == HAS_TRAINED )
    ch_printf( ch, "&GTrained&W: Has Trained.\n\r" );
   else
    ch_printf( ch, "&GTrained&W: Not Trained.\n\r" );
  }
...
void do_cset( CHAR_DATA *ch, char *argument )
{
  ...
  if( !str_cmp( arg, "wizlock" ) )
  {
    ...
    if ( sysdata.WIZLOCK )
	send_to_char( "The game has been Wizlocked.\n\r", ch );
    else
	send_to_char( "The game has been UnWizlocked.\n\r", ch );
    return;
  }
...
if(!obj){
	...
	******/
	...
	case ITEM_POTION:
	case ITEM_LOCKER:
                ch_printf( ch, "&GValue[&W0&G] Flags (&W%d&c):&W",obj->value[0]);
                if (obj->value[0] <= 0)
                   ch_printf(ch," None\n\r");
                else 
		 {
		  ...
		  if ( IS_SET(obj->value[0], CONT_LOCKED) )
		   ...
		   ch_printf(ch," Unlocked");
		  send_to_char( "\n\r", ch );
		 }
		ch_printf( ch, "&GValue[&W1,2,3&G] Combination: &W%d %d %d", obj->value[1], obj->value[2], obj->value[3] );
		break;
	case ITEM_PILL:
		...
		break;
	case ITEM_CAMERA:
		ch_printf( ch, "&GValue[&W0&G] Monitored From: &W%d\n\r", obj->value[0] );
		ch_printf( ch, "&GValue[&W1&G] Camera Size: &W" );
	      if( obj->value[1] == 2 )
		ch_printf( ch, "&w&WNormal Size&w&W\n\r" );
	      else if( obj->value[1] == 1 )
		ch_printf( ch, "&w&WLarge Size&w&W\n\r" );
	      else if( obj->value[1] == 3 )
		ch_printf( ch, "&w&WSmall Size&w&W\n\r" );
	      else if( obj->value[1] == 4 )
		ch_printf( ch, "&w&WTiny Size&w&W\n\r" );
	      else
		ch_printf( ch, "&w&WUnknown&w&W\n\r" );
	        break;
	case ITEM_MONITOR:
		ch_printf( ch, "&GValue[&W0&G] Currently Viewing: &W%d\n\r", obj->value[0] );
		break;
	case ITEM_SALVE:
		ch_printf( ch, "&GValue[&W0&G] Spell Level: &W%d\n\r", obj->value[0]);
	...
	case ITEM_WEAPON:
	    ...
	    if ( obj->value[3] ==  0) 
	    	...
	    	ch_printf( ch, "&WBlaster Pistol&W\n\r"      );
	    else if ( obj->value[3] ==  WEAPON_BLASTER_RIFLE)
		ch_printf( ch, "&WBlaster Rifle&W\n\r"	);
	    else if ( obj->value[3] ==  WEAPON_HEAVY )
		ch_printf( ch, "&WHeavy Blaster Weapon&W\n\r" );
	    else if ( obj->value[3] ==  WEAPON_SIMPLE )
		ch_printf( ch, "&WSimple Weapon&W\n\r" );
	    else if ( obj->value[3] ==  WEAPON_PRIMITIVE )
		ch_printf( ch, "&WPrimitive WEapon&W\n\r" );
	    ...
	    break;
	case ITEM_CRYSTAL:
	    ch_printf( ch, "&GValue[&W0&G] Crystal Type: &W%s", crystal_name[obj->value[0]] );
	    break;
	case ITEM_MEDPAC:
	    ch_printf( ch, "&GValue[&W0&G] Medpac Type: " );
	    if( obj->value[0] == 0 )
		ch_printf( ch, "&WBandage&W\n\r" );
	    else if( obj->value[0] == 1 )
		ch_printf( ch, "&WTravel Medpac&W\n\r" );
	    else if( obj->value[0] == 2 )
		ch_printf( ch, "&WCombat Medpac&W\n\r" );
	    else if( obj->value[0] == 3 )
		ch_printf( ch, "&WEmergency Medkit&W\n\r" );
	    else if( obj->value[0] == 4 )
		ch_printf( ch, "&WMedical Medkit&W\n\r" );
	    else if( obj->value[0] == 5 )
		ch_printf( ch, "&WBacta Vial&W\n\r" );
	    else if( obj->value[0] == 6 )
		ch_printf( ch, "&WBacta Injection&W\n\r" );
	    else
		ch_printf( ch, "&WUnknown&W\n\r" );
	    break;
	case ITEM_ARMOR:
	    ch_printf( ch, "&GValue[&W0&G] Current AC: &W%d\n\r", obj->value[0]);
	    ch_printf( ch, "&GValue[&W1&G] Original AC: &W%d\n\r", obj->value[1]);
	    ch_printf( ch, "&GValue[&W2&G] Armor Type: " );
	    if( obj->value[2] == CLOTHES_ARMOR )
		ch_printf( ch, "&WClothes&W\n\r" );
	    else if( obj->value[2] == LIGHT_ARMOR )
		ch_printf( ch, "&WLight Armor&W\n\r" );
	    else if( obj->value[2] == MEDIUM_ARMOR )
		ch_printf( ch, "&WMedium Armor&W\n\r" );
	    else if( obj->value[2] == HEAVY_ARMOR )
		ch_printf( ch, "&WHeavy Armor&W\n\r" );
	    else if( obj->value[2] == POWERED_ARMOR )
		ch_printf( ch, "&WPowered Armor&W\n\r" );
	    else
		ch_printf( ch, "&WUnknown Armor Type&W\n\r" );
...
void do_addchange(CHAR_DATA *ch, char *argument)
{
 ...
 echo_to_all( AT_GREEN, "&GNew Change Implemented.&w\n\r", 0 );
 return;
}
...
void do_showquests( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  QUEST_DATA *quest;
  char buf[MAX_STRING_LENGTH];
  ...
  if( argument[0] == '\0' )
   {
	send_to_char( "Show who's quests?\n\r", ch );
	return;
   }
  ...
  if( IS_NPC(victim) )
   {
	send_to_char( "NPCs don't undergo quests.\n\r", ch );
	return;
    }
  if( victim->top_level > ch->top_level )
   {
	send_to_char( "Their god-like powers prevent you from spying on them.\n\r", ch );
	return;
   }
  ch_printf( ch, " &G%-15s  &G%-15s\n\r", "Quest Name", "Status" );
  for( quest = victim->first_quest; quest; quest = quest->next )
   {
	STRFREE( buf );
	if( quest->value == 0 )
	  sprintf( buf, "Incomplete" );
	else
	  sprintf( buf, "Completed" );
	ch_printf( ch, " &w&W%-15s  &r%s&w&W\n\r", quest->name, buf );
	num += 1;
   }
  if( num == 0 )
	send_to_char( "&w&WThey have no active quests.\n\r", ch );
  return;
}
