...
DO_FUN *skill_function( char *name )
{
    ...
    {
    case 'a':
	if ( !str_cmp( name, "do_alchemy" ))		return do_alchemy;
	...
	if ( !str_cmp( name, "do_assignpilot" ))		return do_assignpilot;
	...
	break;
    case 'b':
	if ( !str_cmp( name, "do_battlemind" ))		return do_battlemind;
	if ( !str_cmp( name, "do_buycargo" ))		return do_buycargo;
	if ( !str_cmp( name, "do_bonuses" ))		return do_bonuses;
	if ( !str_cmp( name, "do_bartokk" ))		return do_bartokk;
	if ( !str_cmp( name, "do_backup" ))		return do_backup;
	...
	if ( !str_cmp( name, "do_buytroops" ))		return do_buytroops;
	if ( !str_cmp( name, "do_buyhome" ))            return do_buyhome; 
	if ( !str_cmp( name, "do_buyship" ))            return do_buyship; 
	if ( !str_cmp( name, "do_barrel_roll" ))	return do_barrel_roll;
	break;
    case 'c':
	if ( !str_cmp( name, "do_callforce" ))		return do_callforce;
	if ( !str_cmp( name, "do_checksenate" ))	return do_checksenate;
	if ( !str_cmp( name, "do_checkmonitor" ))	return do_checkmonitor;
	if ( !str_cmp( name, "do_clanbestow" ))		return do_clanbestow;
	...
	if ( !str_cmp( name, "do_cybernetics" ))	return do_cybernetics;
	if ( !str_cmp( name, "do_codecrack" ))          return do_codecrack; 
	...
	if ( !str_cmp( name, "do_checkwar" ))		return do_checkwar;
	if ( !str_cmp( name, "do_checkprints" ))	return do_checkprints;
	...
	if ( !str_cmp( name, "do_checkbeacons"))	return do_checkbeacons;
	...
	if ( !str_cmp( name, "do_clanbuytroops" ))      return do_clanbuytroops; 
	if ( !str_cmp( name, "do_clansalvage" ))        return do_clansalvage; 
	if ( !str_cmp( name, "do_clangiveship" ))       return do_clangiveship; 
	...
	if ( !str_cmp( name, "do_contract" ))		return do_contract;
	...
	break;
    case 'd':
	if ( !str_cmp( name, "do_dumpcargo" ))		return do_dumpcargo;
	if ( !str_cmp( name, "do_drainenergy" ))	return do_drainenergy;
	if ( !str_cmp( name, "do_drainforce" ))		return do_drainforce;
	if ( !str_cmp( name, "do_duck" ))		return do_duck;
 	if ( !str_cmp( name, "do_dfeats" ))		return do_dfeats;
	if ( !str_cmp( name, "do_dock" ))		return do_dock;
	if ( !str_cmp( name, "do_demote" ))		return do_demote;
	if ( !str_cmp( name, "do_disableship" ))		return do_disableship;
	if ( !str_cmp( name, "do_deny" ))		return do_deny;
	if ( !str_cmp( name, "do_description" ))	return do_description;
	if ( !str_cmp( name, "do_describe" ))		return do_describe;
//	if ( !str_cmp( name, "do_designship" ))		return do_designship;
	...
	if ( !str_cmp( name, "do_deception" ))          return do_deception;
	...
	if ( !str_cmp( name, "do_droptroops" ))		return do_droptroops;
	break;
    case 'e':
	if ( !str_cmp( name, "do_enhancesenses" ))	return do_enhancesenses;
	if ( !str_cmp( name, "do_enhance" ))		return do_enhance;
	if ( !str_cmp( name, "do_east" ))		return do_east;
	if ( !str_cmp( name, "do_embargo" ))		return do_embargo;
	...
	if ( !str_cmp( name, "do_exempt" ))		return do_exempt;
	if ( !str_cmp( name, "do_exits" ))		return do_exits;
	break;
    case 'f':
	if ( !str_cmp( name, "do_forcestrike" ))	return do_forcestrike;
	if ( !str_cmp( name, "do_forcemind" ))		return do_forcemind;
	if ( !str_cmp( name, "do_forcelightning" ))	return do_forcelightning;
	if ( !str_cmp( name, "do_forcegrip" ))		return do_forcegrip;
	if ( !str_cmp( name, "do_forcedefense" ))	return do_forcedefense;
	if ( !str_cmp( name, "do_fear" ))		return do_fear;
	if ( !str_cmp( name, "do_farseeing" ))		return do_farseeing;
	if ( !str_cmp( name, "do_ffeats" ))		return do_ffeats;
        if ( !str_cmp( name, "do_fskills" ))		return do_fskills;
 	if ( !str_cmp( name, "do_feats" ))		return do_feats;
	...
	if ( !str_cmp( name, "do_fset" ))		return do_fset;
	...
	if ( !str_cmp( name, "do_freeship" ))		return do_freeship;
	if ( !str_cmp( name, "do_fslay" ))		return do_fslay;
	break;
    case 'g':
	if ( !str_cmp( name, "do_gag" ))		return do_gag;
	if ( !str_cmp( name, "do_gatherinfo")) 		return do_gatherinfo;
	if(!str_cmp(name, "do_gatherclans")) return do_gatherclans;
	if ( !str_cmp( name, "do_get" ))		return do_get;
	if ( !str_cmp( name, "do_gemcutting" ))		return do_gemcutting;
	if ( !str_cmp( name, "do_generate_market"))	return do_generate_market;
	if ( !str_cmp( name, "do_give" ))		return do_give;
	if ( !str_cmp( name, "do_giveship" ))		return do_giveship;
	if ( !str_cmp( name, "do_giveslug" ))		return do_giveslug;
...
//	if ( !str_cmp( name, "do_gotoship" ))		return do_gotoship;
	...
	if ( !str_cmp( name, "do_hale" ))		return do_hale;
	...
	if ( !str_cmp( name, "do_holonet" ))		return do_holonet;
	...
	if ( !str_cmp( name, "do_idealog" ))		return do_idealog;
	...
	if ( !str_cmp( name, "do_jedicouncil"))		return do_jedicouncil;
	if ( !str_cmp(name, "do_juke"))		return do_juke;
	...
	break;
    case 'l':
	if ( !str_cmp( name, "do_listcargo" ))		return do_listcargo;
	if ( !str_cmp( name, "do_lockercode" ))		return do_lockercode;
	if ( !str_cmp( name, "do_linkcamera" ))		return do_linkcamera;
	if ( !str_cmp( name, "do_lean" ))		return do_lean;
	...
	if ( !str_cmp( name, "do_launch2"))		return do_launch2;
	...
	if ( !str_cmp( name, "do_makecybernetic" ))	return do_makecybernetic;
	if ( !str_cmp( name, "do_makesith" ))		return do_makesith;
	if ( !str_cmp( name, "do_makecamera" ))		return do_makecamera;
	if ( !str_cmp( name, "do_makesenate" ))		return do_makesenate;
	...
	if ( !str_cmp( name, "do_makecommsystem" ))		return do_makecommsystem;
	if ( !str_cmp( name, "do_makedatapad" ))		return do_makedatapad;
	if ( !str_cmp( name, "do_makearmor" ))		return do_makearmor;
	if ( !str_cmp( name, "do_marry" ))		return do_marry;
	if (!str_cmp(name, "do_makebeacon"))	return do_makebeacon;
	if (!str_cmp(name, "do_makebug"))	return do_makebug;
	if ( !str_cmp( name, "do_make_master" ))        return do_make_master;
	...
	if ( !str_cmp( name, "do_makeshipbomb" ))	return do_makeshipbomb;
	...
	if ( !str_cmp( name, "do_makepike" ))		return do_makepike;
	...
	if ( !str_cmp( name, "do_makebinders" ))	return do_makebinders;
	if ( !str_cmp( name, "do_makegoggles"))		return do_makegoggles;
	...
	if ( !str_cmp( name, "do_makeprototypeship" ))  return do_makeprototypeship;
	...
	if ( !str_cmp( name, "do_makesimulator" ))     return do_makesimulator;
	if ( !str_cmp( name, "do_makemobship" ))     return do_makemobship;
	...
	if ( !str_cmp( name, "do_mobrepair" ))		return do_mobrepair;
	...
	if ( !str_cmp( name, "do_mpquest" ))		return do_mpquest;
  	if ( !str_cmp( name, "do_mptrials" ))		return do_mptrials;
	...
	if ( !str_cmp( name, "do_nullifybeacons"))	return do_nullifybeacons;
	break;
    case 'o':
...
*/	if ( !str_cmp( name, "do_oinvoke" ))		return do_oinvoke;
	if ( !str_cmp( name, "do_oldmstat" ))		return do_oldmstat;
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	if ( !str_cmp( name, "do_pirate" ))		return do_pirate;
	if ( !str_cmp( name, "do_padawan" ))		return do_padawan;
	if ( !str_cmp( name, "do_plantmonitor" ))	return do_plantmonitor;
	if ( !str_cmp( name, "do_plantcamera" ))	return do_plantcamera;
  	if ( !str_cmp( name, "do_penalizeclan" ))	return do_penalizeclan;
	if ( !str_cmp( name, "do_patron" ))		return do_patron;
	...
	if ( !str_cmp( name, "do_plantbug" ))		return do_plantbug;
        if(!str_cmp(name, "do_plantbeacon"))     return do_plantbeacon;
	...
	if ( !str_cmp( name, "do_rentlocker" ))		return do_rentlocker;
	if ( !str_cmp( name, "do_resources" ))		return do_resources;
	if ( !str_cmp( name, "do_remplanet" ))		return do_remplanet;
  	if ( !str_cmp( name, "do_remcouncil" ))		return do_remcouncil;
	...
	if ( !str_cmp( name, "do_remcontract" ))	return do_remcontract;
	if ( !str_cmp( name, "do_remclan" ))		return do_remclan;
	if ( !str_cmp( name, "do_retune"))		return do_retune;
	...
	if ( !str_cmp( name, "do_removebug" ))		return do_removebug;
	...
	if ( !str_cmp( name, "do_removeship" ))           return do_removeship; 
	...
	if ( !str_cmp( name, "do_rpconvert" ))		return do_rpconvert;
	...
	break;
    case 's':
	if ( !str_cmp( name, "do_secret" ))		return do_secret;
	if ( !str_cmp( name, "do_sithsorcery" ))	return do_sithsorcery;
	if ( !str_cmp( name, "do_showquests" ))		return do_showquests;
	if ( !str_cmp( name, "do_setreputation" ))	return do_setreputation;
	if ( !str_cmp( name, "do_shiplist" ))		return do_shiplist;
	if ( !str_cmp( name, "do_setsith" ))		return do_setsith;
	if ( !str_cmp( name, "do_sith" ))		return do_sith;
	if ( !str_cmp( name, "do_settaxrate" ))		return do_settaxrate;
	if ( !str_cmp( name, "do_setgovernor" ))	return do_setgovernor;
	if ( !str_cmp( name, "do_setbonus" ))		return do_setbonus;
	if ( !str_cmp( name, "do_sellcargo" ))		return do_sellcargo;
	if ( !str_cmp( name, "do_scanship" ))		return do_scanship;
	if ( !str_cmp( name, "do_showjedi" ))		return do_showjedi;
	if ( !str_cmp( name, "do_showsenate" ))		return do_showsenate;
	if ( !str_cmp( name, "do_setsenate" ))		return do_setsenate;
	if ( !str_cmp( name, "do_setjedi" ))		return do_setjedi;
	if ( !str_cmp( name, "do_specialty" ))		return do_specialty;
	...
	if ( !str_cmp( name, "do_scanbugs" ))		return do_scanbugs;
	if ( !str_cmp( name, "do_score" ))		return do_score;
	if ( !str_cmp( name, "do_screenname" ))		return do_screenname;
	...
	if ( !str_cmp( name, "do_setinfrared" ))	return do_setinfrared;
	if ( !str_cmp( name, "do_setplanet" ))	        return do_setplanet;
	if ( !str_cmp( name, "do_setwage" ))	        return do_setwage;
	if ( !str_cmp( name, "do_shipmaintenance" ))    return do_shipmaintenance;
	if ( !str_cmp( name, "do_shiprepair" ))         return do_shiprepair; 
	...
	if ( !str_cmp( name, "do_showcontracts" ))	return do_showcontracts;
	if ( !str_cmp( name, "do_showbugs" ))		return do_showbugs;
	if ( !str_cmp( name, "do_showbeacons"))   return do_showbeacons;
	...
	if ( !str_cmp( name, "do_showsocial" ))         return do_showsocial;
	...
	if ( !str_cmp( name, "do_showmodules" ))	return do_showmodules;
	if ( !str_cmp( name, "do_shiplock" ))		return do_shiplock;
	if ( !str_cmp( name, "do_salvage" ))		return do_salvage;
	...
	if ( !str_cmp( name, "do_slog" ))		return do_slog;
	...
	if ( !str_cmp( name, "do_std" ))		return do_std;
	...
	if ( !str_cmp( name, "do_tunemonitor" ))	return do_tunemonitor;
	if ( !str_cmp( name, "do_tellsnoop" ))		return do_tellsnoop;
	...
	if ( !str_cmp( name, "do_unlock" ))		return do_unlock;
        ...
        if ( !str_cmp( name, "do_unoutlaw" ))           return do_unoutlaw;
	...
	if ( !str_cmp( name, "do_verpine" ))		return do_verpine;
	...
	if ( !str_cmp( name, "do_whoinvis" ))		return do_whoinvis;
	if ( !str_cmp( name, "do_wwwimage" ))		return do_wwwimage;
...
{   
    ...
    if ( skill == do_alchemy )		return "do_alchemy";
    ...
    if ( skill == do_bonuses )		return "do_bonuses";
    if ( skill == do_challenge )        return "do_challenge";
    if ( skill == do_cover )		return "do_cover";
    if ( skill == do_clanbestow )	return "do_clanbestow";
    if ( skill == do_checksenate )	return "do_checksenate";
    if ( skill == do_callforce )	return "do_callforce";
    if ( skill == do_checkmonitor )	return "do_checkmonitor";
    ...
    if ( skill == do_gatherinfo )     return "do_gatherinfo";
    if ( skill == do_gatherclans)	return "do_gatherclans";
    if ( skill == do_generate_market)		return "do_generate_market";
    if ( skill == do_add_patrol )     return "do_add_patrol";
    if ( skill == do_special_forces )     return "do_special_forces";
    if ( skill == do_setbonus )		return "do_setbonus";
    if ( skill == do_setjedi )	return "do_setjedi";
    if ( skill == do_setsenate )	return "do_setsenate";
    if ( skill == do_showsenate )	return "do_showsenate";
    if ( skill == do_showjedi )		return "do_showjedi";
    if ( skill == do_jail )     return "do_jail";
    if ( skill == do_elite_guard )     return "do_elite_guard";
    if ( skill == do_embargo )		return "do_embargo";
    ...
    if ( skill == do_barrel_roll ) return "do_barrel_roll";
    if ( skill == do_juke )	return "do_juke";
    if ( skill == do_jedicouncil)	return "do_jedicouncil";
    if ( skill == do_duck )		return "do_duck";
    if ( skill == do_dumpcargo )	return "do_dumpcargo";
    if ( skill == do_drainenergy )	return "do_drainenergy";
    if ( skill == do_drainforce )	return "do_drainforce";
    if ( skill == do_lean )		return "do_lean";
    if ( skill == do_listcargo )	return "do_listcargo";
    if ( skill == do_lockercode )	return "do_lockercode";
    if ( skill == do_linkcamera )	return "do_linkcamera";
    ...
    if ( skill == do_setinfrared )   return "do_setinfrared";
    ...
    if ( skill == do_dfeats )		return "do_dfeats";
    ...
    if ( skill == do_deception )     return "do_deception";
    ...
    if ( skill == do_rentlocker )	return "do_rentlocker";
    if ( skill == do_resources )	return "do_resources";
    if ( skill == do_remplanet )	return "do_remplanet";
    if ( skill == do_rcopy )	return "do_rcopy";
    if ( skill == do_remcouncil ) 	return "do_remcouncil";
    if ( skill == do_recall )    return "do_recall";
    if ( skill == do_buyship )		return "do_buyship";
    if ( skill == do_buytroops )	return "do_buytroops";
    if ( skill == do_buyhome )		return "do_buyhome";
    if ( skill == do_clanbuyship )		return "do_clanbuyship";
    if ( skill == do_clangiveship )		return "do_clangiveship";
    if ( skill == do_clansalvage )		return "do_clansalvage";
    ...
    if ( skill == do_remcontract )         return "do_remcontract";
    if ( skill == do_remclan )         return "do_remclan";
    if ( skill == do_shiprepair )         return "do_shiprepair";
    ...
    if ( skill == do_removeship )       return "do_removeship";
    if ( skill == do_removebug )        return "do_removebug";
    ...
    if ( skill == do_bartokk )		return "do_bartokk";
    if ( skill == do_buycargo )		return "do_buycargo";
    if ( skill == do_battlemind )	return "do_battlemind";
    if ( skill == do_backup )		return "do_backup";
    if ( skill == do_bomb )		return "do_bomb";
    ...
    if ( skill == do_checkwar )		return "do_checkwar";
    if ( skill == do_checkprints )	return "do_checkprints";
    if ( skill == do_checkbeacons )     return "do_checkbeacons";
    ...
    if ( skill == do_contract )		return "do_contract";
    ...
    if ( skill == do_describe )		return "do_describe";
//    if ( skill == do_designship )     return "do_designship";
    ...
    if ( skill == do_showbugs )     return "do_showbugs";    
    if ( skill == do_showbeacons )  return "do_showbeacons";
...
/*    if ( skill == do_diagnose )		return "do_diagnose"; */
    ...
    if ( skill == do_exempt )		return "do_exempt";
    if ( skill == do_exits )		return "do_exits";
    if ( skill == do_fskills )		return "do_fskills";
    if ( skill == do_feats )		return "do_feats";
    if ( skill == do_ffeats )		return "do_ffeats";
    if ( skill == do_farseeing )	return "do_farseeing";
    if ( skill == do_fear )		return "do_fear";
    if ( skill == do_forcedefense )	return "do_forcedefense";
    if ( skill == do_forcegrip )	return "do_forcegrip";
    if ( skill == do_forcelightning )	return "do_forcelightning";
    if ( skill == do_forcestrike )	return "do_forcestrike";
    if ( skill == do_forcemind )	return "do_forcemind";
    ...
    if ( skill == do_fset )		return "do_fset";
    ...
    if ( skill == do_freeship )		return "do_freeship";
    if ( skill == do_hale )		return "do_hale";
    ...
    if ( skill == do_giveslug )		return "do_giveslug";
...
*/    if ( skill == do_gtell )		return "do_gtell";
    ...
    if ( skill == do_holonet )  	return "do_holonet";
    ...
    if ( skill == do_idealog )		return "do_idealog";
    ...
    if ( skill == do_launch2 )		return "do_launch2";
    ...
    if ( skill == do_makesenate )	return "do_makesenate";
    if ( skill == do_makecybernetic )	return "do_makecybernetic";
    if ( skill == do_meditate )		return "do_meditate";
    if ( skill == do_makesith )		return "do_makesith";
    if ( skill == do_makecamera )	return "do_makecamera";
    ...
    if ( skill == do_makeshipbomb )	return "do_makeshipbomb";
    ...
    if(skill == do_makebeacon)	return "do_makebeacon";
    if(skill == do_makebug)	return "do_makebug";
    if ( skill == do_make_master )	return "do_make_master";
    if ( skill == do_makeblade )	return "do_makeblade";
    if ( skill == do_makeblaster )	return "do_makeblaster";
    if ( skill == do_makebinders)	return "do_makebinders";
    if ( skill == do_makegoggles)	return "do_makegoggles";
    if ( skill == do_makelightsaber )	return "do_makelightsaber";
    if ( skill == do_makepike )		return "do_makepike";
    ...
    if ( skill == do_makesimulator )   return "do_makesimulator";
    if ( skill == do_makemobship )   return "do_makemobship";
    ...
    if ( skill == do_mobrepair )	return "do_mobrepair";
    ...
    if ( skill == do_mptrials )		return "do_mptrials";
    ...
    if ( skill == do_nullifybeacons )   return "do_nullifybeacons";
...
*/    if ( skill == do_oinvoke )		return "do_oinvoke";
    if ( skill == do_oldmstat )		return "do_oldmstat";
...
/*  if ( skill == do_opentourney )	return "do_opentourney"; */
    ...
    if ( skill == do_patron )		return "do_patron";
    if ( skill == do_pirate )		return "do_pirate";
    if ( skill == do_promote )		return "do_promote";
    if ( skill == do_padawan )		return "do_padawan";
    if ( skill == do_plantmonitor )	return "do_plantmonitor";
    if ( skill == do_plantcamera )	return "do_plantcamera";
    if ( skill == do_penalizeclan )	return "do_penalizeclan";
    if ( skill == do_pfiles )		return "do_pfiles";
    if ( skill == do_plantbug )		return "do_plantbug";
    if ( skill == do_plantbeacon )      return "do_plantbeacon";
    ...
    if ( skill == do_rpconvert )	return "do_rpconvert";
    ...
    if ( skill == do_specialty )	return "do_specialty";
    if ( skill == do_secret )		return "do_secret";
    if ( skill == do_sithsorcery )	return "do_sithsorcery";
    if ( skill == do_showquests )	return "do_showquests";
    if ( skill == do_setreputation )	return "do_setreputation";
    if ( skill == do_shiplist )		return "do_shiplist";
    if ( skill == do_setsith )		return "do_setsith";
    if ( skill == do_sith )		return "do_sith";
    if ( skill == do_settaxrate )	return "do_settaxrate";
    if ( skill == do_setgovernor )	return "do_setgovernor";
    if ( skill == do_scanship )		return "do_scanship";
    if ( skill == do_sellcargo )	return "do_sellcargo";
    ...
    if ( skill == do_scanbugs )		return "do_scanbugs";
    if ( skill == do_score )		return "do_score";
    if ( skill == do_screenname )	return "do_screenname";
    ...
    if ( skill == do_shipmaintenance )  return "do_shipmaintenance";
    ...
    if ( skill == do_showcontracts )	return "do_showcontracts";
    if ( skill == do_showship )         return "do_showship";
    if ( skill == do_showplanet )         return "do_showplanet";
    if ( skill == do_showsocial )       return "do_showsocial";
    ...
    if ( skill == do_showmodules )	return "do_showmodules";
    if ( skill == do_shiplock )		return "do_shiplock";
    if ( skill == do_salvage )		return "do_salvage";
    ...
    if ( skill == do_slog )		return "do_slog";
    ...
    if ( skill == do_std )		return "do_std";
    if ( skill == do_switch )		return "do_switch";
    if ( skill == do_tamp )		return "do_tamp";
    if ( skill == do_tell )		return "do_tell";
    if ( skill == do_tellsnoop )	return "do_tellsnoop";
    if ( skill == do_makecommsystem )	return "do_makecommsystem";
    if ( skill == do_makedatapad )	return "do_makedatapad";
    if ( skill == do_codecrack )	return "do_codecrack";
    if ( skill == do_cybernetics )	return "do_cybernetics";
    if ( skill == do_disableship )	return "do_disableship";
    ...
    if ( skill == do_whoinvis )		return "do_whoinvis";
    ...
    if ( skill == do_wwwimage )		return "do_wwwimage";
...
SKILLTYPE *fread_skill( FILE *fp )
{
	...
	case 'C':
		...
		fMatch = TRUE;
		if ( (spellfun=spell_function(w)) != NULL )
