...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if ( IS_OBJ_STAT(obj, ITEM_IMBUED)	  )   strcat( buf, "(Imbued) "    );
    if ( IS_OBJ_STAT(obj, ITEM_IGNITED)	  )   strcat( buf, "(Ignited) "   );
    if ( IS_IMMORTAL(ch)
	 && IS_OBJ_STAT(obj, ITEM_PROTOTYPE) ) strcat( buf, "(PROTO)&w&W ");
...
void show_list_to_char( OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing )
{
	...
	* (IS_NPC(ch) ? 1 : (ch->pcdata->condition[COND_DRUNK] ? (ch->pcdata->condition[COND_DRUNK]/12) : 1));
	...
	if ( !IS_IMMORTAL(ch) && obj->item_type == ITEM_CAMERA && !can_see_obj( ch, obj ) )
	  continue;
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( IS_DARK(victim) && can_see_force( ch, victim)) strcat( buf, "&w&W(&RDark&w&W) " );
    if ( HAS_FEAT( victim, FORCE_SENSITIVE ) && !IS_DARK(victim) && can_see_force( ch, victim ) )
						strcat( buf, "&w&W(&CLight&w) " );
    if ( IS_NPC(victim) && victim->alignment > 0 && can_see_force( ch, victim))
						strcat( buf, "(&CLight&w) " );
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	&&   can_see_obj( ch, obj ) )
	{
	    if ( !IS_IMMORTAL(ch) && obj && obj->item_type == ITEM_WEAPON && obj->value[3] == WEAPON_LIGHTSABER && !IS_OBJ_STAT(obj, ITEM_IGNITED) )
		continue;
	    if ( !found )
	    {
		send_to_char( "\n\r", ch );
    ...
    if ( 20 < ( number_range(1, 20) + ch->skill_level[SPOT_SKILL] + stat_table[get_curr_wis(ch)].mod) )
    {
			 ...
			 if(ship->class != 11)
			  ...
			  ch_printf(ch, "&O%s\n\r", target->name); }
                   ...
                   {        
    			        ...
    			        ( missile->missiletype ==  PROTON_TORPEDO ? "A Proton Torpedo" : 
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	&&  !IS_SET(pexit->exit_info, EX_HIDDEN) )
	{
		...
		if ( IS_SET(pexit->exit_info, EX_CLOSED) )
		{
		    sprintf( buf + strlen(buf), "&W&w%-5s - (closed)\n\r",
		    capitalize( dir_name[pexit->vdir] ) );
		}
		else if ( IS_SET(pexit->exit_info, EX_WINDOW) )
		{
		    sprintf( buf + strlen(buf), "&W&w%-5s - (window)\n\r",
		    capitalize( dir_name[pexit->vdir] ) );
		}
		else if ( IS_SET(pexit->exit_info, EX_xAUTO) )
		{
		   sprintf( buf + strlen(buf), "&W&w%-5s - &W&w%s\n\r",
		    ...
		    sprintf( buf + strlen(buf), "&W&w%-5s - %s\n\r",
		    capitalize( dir_name[pexit->vdir] ),
		    room_is_dark( pexit->to_room )
			?  "Too dark to tell"
			: pexit->to_room->name );
...
*/
  ...
  if (ch && ch->pcdata && ch->pcdata->clan && wch->pcdata->clan 
   ...
   if ( wch->desc && wch->desc->connected == CON_EDITING )
	sprintf( buf, "%s  &w&W(Writing) %s%s%s%s %s%s\n%s",
	    race,
	    invis_str,
     ...
     * display the information and clean up.26
     */
    ...
    {
      if ( !ch )
        fprintf( whoout,htmlcolor("&G[&g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&G><&r-&g><&r-&g> &G[ &WRotS &G][ &WMortals &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&G><&r-&g><&r-&g>&G]&W\n\n") );
      else
       send_to_pager( "\n\r&G[&g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&G><&r-&g><&r-&g> &G[ &WRotS &G][ &WMortals &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&G><&r-&g><&r-&g>&G]&W\n\r\n\r", ch );
    }
    ...
    {
      if ( !ch )
        fprintf( whoout,htmlcolor("&G[&r-&g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g> &G[ &CReturn Of The Sith &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g>&r-&G]&W\n\n") );
      else
       send_to_pager( "\n\r&G[&r-&g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g> &G[ &CReturn Of The Sith &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g>&r-&G]&W\n\r\n\r", ch );
    }
    ...
    if ( first_imm )
    {
      if ( !ch )
       ...
       send_to_pager(  "\n\r&G[&r-&g<&r-&g>&g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g> &G[ &CReturn Of The Sith &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g>&r-&G]&W\n\r\n\r", ch );
    }
    ...
    if ( !ch )
    {
	fprintf( whoout, htmlcolor("&G[ &r%d &Wvisible &Wplayer%s. &r%d &Wtotal&W player%s. &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g>&W\n"),
	...
	fclose( whoout );
	ch_printf( ch, "\n\r&G[ &r%d &Wvisible &Wplayer%s. &r%d &Wtotal&W player%s. &G] &g<&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g><&r-&g>&W\n\r",
		sMatch, sMatch == 1 ? "" : "s", nMatch, nMatch == 1 ? "" : "s" );
    return;
}
...
void do_setrank( CHAR_DATA *ch, char *argument )
{
	...
	if(ch->pcdata->clan == NULL ) 
        { 
 	        send_to_char("You must be in a clan to use SETRANK.\n\r",ch); 
	        return; 
        } 
	...
	if(vict->pcdata->clan == NULL ) 
        { 
       		send_to_char("You may only setrank on someone in your clan.\n\r",ch);
	        return; 
        } 
	...
	{
		if(vict->rank) STRFREE(vict->rank);
		vict->rank = STRALLOC("                  ");
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( !IS_SET( ch->deaf, CHANNEL_TELLS )
        ...
        send_to_char( !IS_SET( ch->deaf, CHANNEL_BARTOKK )
            ? " +BARTOKK"
            : " -bartokk",
            ch );
	...
	send_to_char( !IS_SET(ch->deaf, CHANNEL_VERPINE)
	    ? " +VERPINE"
	    : " -verpine",
	    ch );
	...
	else
	{
	...
	else if ( !str_cmp( arg+1, "verpine"  ) ) bit = CHANNEL_VERPINE;
        ...
        else if ( !str_cmp( arg+1, "bartokk"  ) ) bit = CHANNEL_BARTOKK;
...
void do_whois( CHAR_DATA *ch, char *argument)
{
  ...
  if( !IS_IMMORTAL(ch) && !has_datapad )
   {
	send_to_char( "Without a datapad how will you get this information?\n\r", ch );
	return;
   }
  ...
  ch_printf(ch, "&G%s &w&Wis a &G%s %s",
	victim->name,
	victim->sex == SEX_MALE ? "male" : 
	victim->sex == SEX_FEMALE ? "female" : "neutral",
	npc_race[victim->race]);
  if (IS_IMMORTAL(ch))
    ch_printf(ch, " &w&Win room &G%d&w&W.\n\r",
	victim->in_room->vnum);
  else
    ch_printf(ch, "&w&W.\n\r");
  ...
  if( HAS_FEAT( ch, INFAMY_FEAT ) )
   {
     if( ch->reputation >= 0 && ch->reputation < 3 )
	sprintf( rname, "Never heard of %s.", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 3 && ch->reputation < 5 )
	sprintf( rname, "%s is known in a system or two.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
     else if( ch->reputation >= 5 && ch->reputation < 7 )
	sprintf( rname, "%s is wanted in many star systems.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
     else if( ch->reputation >= 7 && ch->reputation < 9 )
	sprintf( rname, "%s is feared in a few systems.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
     else if( ch->reputation >= 9 && ch->reputation < 12 )
	sprintf( rname, "Rumors of %s are echoed throughout the universe.", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 12 )
	sprintf( rname, "%s is feared throughout the universe.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
   }
  else if( HAS_FEAT( ch, FAME_FEAT ) )
   {
     if( ch->reputation >= 0 && ch->reputation < 3 )
	sprintf( rname, "Never heard of %s.", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 3 && ch->reputation < 5 )
	sprintf( rname, "%s people sing tales about %s.", ch->sex == SEX_MALE ? "His" : ch->sex == SEX_FEMALE ? "Her" : "Its", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 5 && ch->reputation < 7 )
	sprintf( rname, "%s is loved on quite a few civilized planets.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
     else if( ch->reputation >= 7 && ch->reputation < 9 )
	sprintf( rname, "%s name is a household word across the universe.", ch->sex == SEX_MALE ? "His" : ch->sex == SEX_FEMALE ? "Her" : "Its" );
     else if( ch->reputation >= 9 && ch->reputation < 12 )
	sprintf( rname, "%s is highly sought after for %s expertise.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It", ch->sex == SEX_MALE ? "his" : ch->sex == SEX_FEMALE ? "her" : "its" );
     else if( ch->reputation >= 12 )
	sprintf( rname, "%s face is well known throughout the universe.", ch->sex == SEX_MALE ? "His" : ch->sex == SEX_FEMALE ? "Her" : "Its" );
   }
  else
   {
     if( ch->reputation >= 0 && ch->reputation < 6 )
	sprintf( rname, "Never heard of %s.", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 6 && ch->reputation < 10 )
	sprintf( rname, "People in the right channels may of heard of %s.", ch->sex == SEX_MALE ? "him" : ch->sex == SEX_FEMALE ? "her" : "it" );
     else if( ch->reputation >= 10 )
	sprintf( rname, "%s may just be a legend.", ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It" );
   }
  
  ch_printf( ch, "&G%s's &w&Wreputation:\n\r&G%s&w&W\n\r", victim->name, rname );
  

  if(victim->pcdata->homepage && victim->pcdata->homepage[0] != '\0')
    ch_printf(ch, "&G%s's &w&Whomepage can be found at &G%s&w&W.\n\r", 
	victim->name,
	victim->pcdata->homepage);

  if(victim->pcdata->bio && victim->pcdata->bio[0] != '\0')
    ch_printf(ch, "&G%s's &w&Wpersonal bio:\n\r%s",
	victim->name,
	victim->pcdata->bio);
  ...
  {  
	...
	ch_printf(ch, "&G%s &w&Wwas authorized by &G%s&w&W.\n\r",
		victim->name, victim->pcdata->authed_by);

    ch_printf(ch, "&G%s &w&Whas killed &G%d &w&Wmobiles, and been killed by a mobile &G%d &w&Wtimes.\n\r",
		victim->name, victim->pcdata->mkills, victim->pcdata->mdeaths );
    if ( victim->pcdata->pkills || victim->pcdata->pdeaths )
	ch_printf(ch, "&G%s &w&Whas killed &G%d &w&Wplayers, and been killed by a player &G%d &w&Wtimes.\n\r",
		victim->name, victim->pcdata->pkills, victim->pcdata->pdeaths );
    if ( victim->pcdata->illegal_pk )
	ch_printf(ch, "&G%s &w&Whas committed &G%d &w&Willegal player kills.\n\r",
		victim->name, victim->pcdata->illegal_pk );

    ch_printf(ch, "&G%s &w&Wis &r%s&w&Whelled at the moment.\n\r",
	victim->name, 
	(victim->pcdata->release_date == 0) ? "not " : "");

    if(victim->pcdata->release_date != 0)
      ch_printf(ch, "&G%s &w&Wwas helled by &G%s&w&W, and will be released on &G%24.24s&w&W.\n\r",
    ...
    {
      sprintf(buf2, "&w&WThis player has the following flags set:&G");
    ...
    {
      sprintf (buf2, "&G%s's &w&WIP info: &G%s ", victim->name, victim->desc->hostip);
    ...
    {
        sprintf (buf2, "&w&WEmail: &G%s&w&W\n\r" , victim->pcdata->email );
        send_to_char(buf2, ch);
    }
    if ( victim->pcdata->screenname && victim->pcdata->screenname[0] != '\0' )
    {
	sprintf (buf2, "&w&WAim Name: &G%s&w&W\n\r", victim->pcdata->screenname );
	send_to_char(buf2, ch);
    }
  }
}
...
bool can_see_force( CHAR_DATA *ch, CHAR_DATA *victim )
{
  int chance, resist;
  ...
  if( !IS_NPC(ch) && !HAS_FEAT(ch, FORCE_SENSITIVE) )
	return FALSE;

  if( IS_NPC(ch) )
	chance = 18;
  else
	chance = ( number_range( 1, 20 ) + ch->force_level[SEE_FORCE] + stat_table[get_curr_wis(ch)].mod );

  if( IS_NPC(victim) )
	resist = 18;
  else
	resist = ( number_range( 1, 20 ) + victim->force_level[FORCE_STEALTH] + stat_table[get_curr_cha(victim)].mod );
