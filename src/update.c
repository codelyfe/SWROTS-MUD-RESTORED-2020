...
int max_level( CHAR_DATA *ch, int ability)
{
    ...
    if ( ability == FRINGER_ABILITY || ability == NOBLE_ABILITY || ability == SCOUT_ABILITY
      || ability == SCOUNDREL_ABILITY || ability == SOLDIER_ABILITY || ability == TECH_SPEC_ABILITY
      || ability == FORCE_ADEPT_ABILITY || ability == JEDICONSULAR_ABILITY || ability == JEDIGUARDIAN_ABILITY )
	level = 20;

    if ( ability == BOUNTYHUNTER_ABILITY || ability == CRIMELORD_ABILITY || ability == DEVOTEE_ABILITY
      || ability == MARAUDER_ABILITY || ability == TROOPER_ABILITY || ability == JEDIACE_ABILITY
      || ability == INVESTIGATOR_ABILITY || ability == JEDIMASTER_ABILITY || ability == STARSHIPACE_ABILITY
      || ability == ENGINEER_ABILITY || ability == INFILTRATOR_ABILITY || ability == PROTECTOR_ABILITY
      || ability == MARTIALARTS_ABILITY || ability == SLICER_ABILITY || ability == WITCH_ABILITY
      || ability == ACOLYTE_ABILITY || ability == SITHLORD_ABILITY || ability == SITHWARRIOR_ABILITY 
      || ability == MYSTICAGENT_ABILITY || ability == AEROBAT_ABILITY )
	level = 10;

    if ( ability == ANTARIAN_ABILITY || ability == ENFORCER_ABILITY || ability == VIGO_ABILITY
      || ability == BROKER_ABILITY || ability == NOBLELORD_ABILITY || ability == ESPIONAGE_ABILITY || ability == BERSERKER_ABILITY )
	level = 5;
...
void advance_level( CHAR_DATA *ch , int ability)
{
    ...
    if( ch->top_level >= 30 )
     {
	send_to_char( "&wYou have already maxed out your level.\n\r", ch );
	return;
     }
    if( ch->top_level > 0 && ch->experience < exp_table[ch->top_level+1].exp )
     {
	ch_printf(ch, "&wYou still need &r%d&w experience before you can level.\n\r", (exp_table[ch->top_level+1].exp - ch->experience) );
	return;
     }
    if( ch->class_level[ability] >= max_level(ch, ability) )
     {
	send_to_char( "&wYou have already mastered that ability.\n\r", ch );
	return;
     }
    ch->top_level += 1;
    ch->forcepoints += 1;
    if( ch->forcepoints >5 && !HAS_FEAT( ch, FORCE_SENSITIVE ) )
	ch->forcepoints = 5;
    if( IS_DROID( ch ) )
	ch->forcepoints = 0;
    if( ch->top_level == 1 || ch->top_level == 3 || ch->top_level == 6 ||
        ch->top_level == 9 || ch->top_level == 12 || ch->top_level == 15 ||
	ch->top_level == 18 || ch->top_level == 21 || ch->top_level == 24 ||
	ch->top_level == 27 || ch->top_level == 30 )
	ch->unused_feats += 1;
    if( ch->top_level == 4 || ch->top_level == 8 || ch->top_level == 12 ||
	ch->top_level == 16 || ch->top_level == 20 || ch->top_level == 24 || ch->top_level == 28 )
	ch->trains += 1;
    if( ch->darkpoints > 0 )
  	gain_darklevel(ch);
    level_class(ch, ability);
    return;
}   
...
void gain_exp( CHAR_DATA *ch, int gain )
{
  ...
  if( ch->experience >= exp_table[ch->top_level+1].exp )
   {
	ch->experience = exp_table[ch->top_level+1].exp + 1;
	if( ch->position != POS_FIGHTING )
	 ch_printf( ch, "&wYou have enough experience to gain level &Y%d&w!\n\r", ch->top_level+1 );
        return;
   }
  if( ch->pcdata && ch->pcdata->clan )
   {
	...
	if( !clan )
	  return;
	clan->experience += gain;
	if( clan->experience >= clan_exp[clan->level].exp )
         {
	   clan->level += 1;
	   sprintf( buf, "%s has gained your clan level %d!", ch->name, clan->level );
	   sprintf( buf2, "You have gained your clan level %d!", clan->level );
	   act( AT_CLAN, buf2, ch, NULL, NULL, TO_CHAR );
		...
		continue;
	 	...
	 	continue;
	     if( d->character->pcdata->clan && !str_cmp( d->character->pcdata->clan->name, ch->pcdata->clan->name) )
	      act( AT_CLAN, buf, ch, NULL, d->character, TO_VICT );
	    }
	 }	     
   }
  return;
...
/*    short drug;
    ...
    for ( drug=0 ; drug <= 9 ; drug ++ )
    {
       ...
       {      
           ...
           retcode = damage(ch, ch, 5, TYPE_UNDEFINED, FALSE);
       }
...
//              gain_addiction( ch );
	...
	if ( !char_died(ch) )
	{
	    ...
	    {
  	      if( ch->in_room->area->planet->atmosphere_type == ATMO_GASEOUS && !IS_AFFECTED(ch, AFF_GAS_BREATH) )
	       {
	      ...
	      if ( !IS_AFFECTED(ch, AFF_HEAT_RESIST) && ch->in_room->area->planet->atmosphere_type == ATMO_HEAT )
	       {
	      ...
	      if ( !IS_AFFECTED(ch, AFF_COLD_RESIST) && ch->in_room->area->planet->atmosphere_type == ATMO_BLIZZARD )
	       {
	    ...
	    if ( !IS_NPC(ch) && time_info.hour == 1 && ch->resources > 0 && ch->resources_temp < 1 )
	    {
	       ch->resources_temp = 1;
	       send_to_char("You may again call upon your Resources.\n\r", ch );
	    }	  
	  ...
	  continue;
              ...
              obj->value[3] != WEAPON_BOWCASTER &&  obj->value[3] != WEAPON_FORCE_PIKE)
        {
           ...
           if ( obj->value[4] <= 0 )
           {
              ...
              else if ( obj->value[3] == WEAPON_VIBRO )
              {
...
void check_force( CHAR_DATA *ch )
{
  ...
  if( !IS_DARK(ch) )
   {
    if( ch->pcdata->trials == HAS_PASSED )
     {
	if( ch->class_level[JEDIMASTER_ABILITY] > 0 )
	  ch->pcdata->force_status = STATUS_JEDI_MASTER;
        else
	  ch->pcdata->force_status = STATUS_JEDI_KNIGHT;
	return;
     }
    else if( ch->pcdata->trials == NOT_PASSED && ( ch->class_level[JEDIGUARDIAN_ABILITY] > 0 || ch->class_level[JEDICONSULAR_ABILITY] >0 ) )
     {
	ch->pcdata->force_status = STATUS_PADAWAN;
	return;
     }
    else
     {
	ch->pcdata->force_status = STATUS_FORCE_SENSITIVE;
	return;
     }
   }
  else if( ch->class_level[SITHLORD_ABILITY] == 0 && ch->class_level[ACOLYTE_ABILITY] == 0 && ch->class_level[SITHWARRIOR_ABILITY] == 0 )
   {
     if( ch->class_level[JEDIGUARDIAN_ABILITY] == 0 && ch->class_level[JEDICONSULAR_ABILITY] == 0 )
      {
	ch->pcdata->force_status = STATUS_FORCE_SENSITIVE;
	return;
      }
     else if( ch->class_level[JEDIMASTER_ABILITY] > 0 )
      {
	ch->pcdata->force_status = STATUS_DARK_MASTER;
 	return;
      }
     else if( ch->pcdata->trials == HAS_PASSED )
      {
	ch->pcdata->force_status = STATUS_DARK_KNIGHT;
 	return;
      }
     else
      {
	ch->pcdata->force_status = STATUS_DARK_PADAWAN;
	return;
      }
   }
 return;
...
void update_med( CHAR_DATA *ch )
{
 ...
 if( IS_DARK(ch) )
  {
    send_to_char( "&w&WYou meditate, reveling in your vile darkness.\n\r", ch );
    return;
  }

 if( !HAS_FEAT( ch, FORCE_SENSITIVE ) )
  {
    number = number_range( 1, 250 );
    if( number == 1 )
     {
	SET_BIT( ch->feats, FORCE_SENSITIVE );
	ch->forcepoints += 1;
	send_to_char( "&BYou awaken from your meditation with a sudden startle.&w\n\r", ch );
	send_to_char( "&BYou have discovered the &CForce&B within you!&w\n\r", ch );
	act( AT_ACTION, "$n awakens from $s meditation with a sudden startle.", ch, NULL, NULL, TO_ROOM );
	return;
     }
    send_to_char( "&w&WThrough meditation you strive to find your inner peace.\n\r", ch );
    return;
  }

 darkness = ch->darkpoints;
 force = ch->forcepoints;
 
 if( darkness < 1 )
  {
    send_to_char( "&w&WThrough meditation your grow closer to the &CForce&w&W\n\r", ch );
    return;
  }
 if( force < 1 )
  {
    send_to_char( "&w&WYou struggle to clense yourself of the &rDark Side&w&W, but you ultimately fail.\n\r", ch );
    return;
  }
 ch->forcepoints -= 1;
 ch->darkpoints -= 1;
 if( ch->darkpoints > 0 )
    send_to_char( "&w&WYou feel the &rDark Side&w&W of the &CForce&w&W release its grip upon you slightly.\n\r", ch );
 else
    send_to_char( "&w&WYou feel clensed of the &rDark Side&w&W of the &CForce&w&W.\n\r", ch );
 return;
}
