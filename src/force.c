...
{
  char *mesg;
  char outp[MAX_STRING_LENGTH];
    mesg = str_dup(msg);
    if(victim)
    {
        mesg = strrep(mesg,"$vfl",force_get_level(victim));
        mesg = strrep(mesg,"$vp",force_get_possessive(victim));
        mesg = strrep(mesg,"$vo",force_get_objective(victim));
        mesg = strrep(mesg,"$vn",force_get_pronoun(victim));
        mesg = strrep(mesg,"$v",PERS(victim, ch));
    }
    if(ch)
    {
        mesg = strrep(mesg,"$nfl",force_get_level(ch));
        mesg = strrep(mesg,"$np",force_get_possessive(ch));
        mesg = strrep(mesg,"$no",force_get_objective(ch));
        mesg = strrep(mesg,"$nn",force_get_pronoun(ch));
        mesg = strrep(mesg,"$n",ch->name);
    }
  if(ch)
  {
    if(ch->force_type == FORCE_JEDI)
    {
        mesg = strrep(mesg,"$FC","&G&w&B");
        mesg = strrep(mesg,"$fc","&G&w&C");
    }
    else if(ch->force_type == FORCE_SITH)
    {
        mesg = strrep(mesg,"$FC","&G&w&R");
        mesg = strrep(mesg,"$fc","&G&w&r");
    }
    else
    {
        mesg = strrep(mesg,"$FC","&G&w&W");
        mesg = strrep(mesg,"$fc","&G&w");
    }
  }
    mesg = strrep(mesg,"$RN$$RN$","\r\n\r\n");
    mesg = strrep(mesg,"$RN$","\r\n");
    sprintf(outp,"&G&w%s&G&w",mesg);
  return str_dup( outp );
}
