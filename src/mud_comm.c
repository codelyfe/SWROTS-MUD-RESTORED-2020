...
void do_mptrials( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    ...
    if( !( victim = get_char_room_mp( ch, argument ) ) )
     {
        progbug( "Mptrials - victim does not exist", ch );
        return;
     }
    if( IS_NPC(victim) )
     	return;
	...
	return;
    victim->pcdata->trials = HAS_PASSED;
...
void do_mpquestcomplete( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Huh?\n\r", ch );
	progbug( "MpQuestcomplete - user use", ch );
	return;
    }
    ...
    if( ( quest = quest_from_name( victim, argument ) ) == NULL )
     {
	progbug( "Mpquestcomplete - Player does not have the quest.", ch );
	return;
     }
    quest->value += 1;
    save_char_obj( victim );
...
void do_mpquest( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
     {
        progbug( "Mpquest - No argument", ch );
        return;
     }
    if( !( victim = get_char_room_mp( ch, arg ) ) )
     {
        progbug( "Mpquest - victim does not exist", ch );
        return;
     }
    if( IS_NPC(victim) )
	return;
	...
	if( !str_cmp( quest->name, argument ) )
	 {
	   progbug( "Mpquest - Already posseses quest", victim );
	   return;
	 }
     }
    CREATE( new_quest, QUEST_DATA, 1 );
    new_quest->name = STRALLOC( argument );
    new_quest->value = 0;
    LINK( new_quest, victim->first_quest, victim->last_quest, next, prev );
    save_char_obj( victim );
    return;
}
